...
const teamColor = { village: '#10b981', werewolf: '#ef4444', solo: '#f59e0b' }
                ...
                <div className="space-y-3">
                  {lobbyPlayers.map((p, i) => (
                    <motion.div key={p.userId || i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                      className={`flex items-center gap-3 p-3 rounded-xl ${p.isBot ? 'bg-cyan-900/30 border border-cyan-500/20' : 'bg-dark-700/50'}`}>
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold ${p.isBot ? 'bg-gradient-to-br from-cyan-500 to-blue-600' : 'bg-gradient-to-br from-wolf-500 to-blood-500'}`}>
              ...
              <div className="card p-6">
                ...
                <div className="space-y-4">
                  ...
                  <motion.button 
                    ...
                    style={{ background: 'linear-gradient(135deg, #EF5350, #FF7043)', color: 'white' }}>
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0" style={{
        background: 'linear-gradient(180deg, #0A1424 0%, #162B4D 40%, #1E3F66 70%, #0F2337 100%)',
      }} />
      ...
      <div className="absolute bottom-0 left-0 w-32 h-48 opacity-20" style={{
        background: 'radial-gradient(ellipse at bottom left, #0d3320 0%, transparent 70%)'
      }} />
      <div className="absolute bottom-0 right-0 w-32 h-48 opacity-20" style={{
        background: 'radial-gradient(ellipse at bottom right, #0d3320 0%, transparent 70%)'
      }} />
      {/* Stars */}
      {[...Array(20)].map((_, i) => (
      ...
      <header className="relative z-20 flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          {/* Currency */}
          <div className="flex items-center gap-4 bg-dark-800/70 backdrop-blur-sm rounded-full px-5 py-2 border border-white/10">
        ...
        <aside className="w-64 p-4 flex flex-col gap-3">
          ...
          <div className="mb-4">
            <h1 className="text-3xl font-black tracking-tight" style={{ fontFamily: 'Outfit', color: '#fff' }}>
              W<span style={{ color: '#F05A88' }}>🐾</span>lvesville
            </h1>
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            style={{ background: 'linear-gradient(135deg, #F05A88, #FF7043)', boxShadow: '0 4px 20px rgba(240,90,136,0.4)' }}>
            CHƠI
          </motion.button>
          ...
          {[
            ...
            <motion.button key={item.key} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              ...
              className={`py-3 px-4 rounded-2xl font-bold text-sm text-dark-800 transition-all ${activeTab === item.key ? 'bg-white shadow-md' : 'bg-white/90 hover:bg-white'}`}>
            ...
            {activeTab === 'play' && (
              <motion.div key="play" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                ...
                <motion.div animate={{ y: [0, -15, 0] }} transition={{ repeat: Infinity, duration: 4 }}
                  ...
                  <img src="/modern-wolf-logo.png" className="w-40 h-40 object-cover rounded-3xl drop-shadow-[0_0_25px_rgba(240,90,136,0.6)] border-2 border-pink-500/30" alt="Wolvesville Logo" />
                </motion.div>
                <h2 className="text-4xl font-black text-white mb-3" style={{ textShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
                  Ma Sói Online
                </h2>
                <p className="text-gray-300 text-lg max-w-md mx-auto mb-6">
                  Chơi cùng bạn bè • 14 vai trò • Chiến đấu trí tuệ
                </p>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={handleCreateRoom}
                  className="py-4 px-10 rounded-2xl font-black text-xl text-white transition-all"
                  style={{ background: 'linear-gradient(135deg, #F05A88, #FF7043)', boxShadow: '0 6px 30px rgba(240,90,136,0.5)' }}>
                  🎮 Tạo Phòng Mới
                </motion.button>
              </motion.div>
            )}
        ...
        <aside className="w-72 p-4 space-y-3">
          ...
          <div className="bg-white/95 rounded-2xl p-4 shadow-lg">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-wolf-500 to-blood-500 flex items-center justify-center font-bold text-white">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-blue-400 to-blue-600 rounded-full transition-all" style={{ width: `${Math.min(((user?.xp || 0) / (user?.xp_next_level || 1000)) * 100, 100)}%` }} />
                  </div>
          ...
          {/* Quick Stats */}
          <div className="bg-dark-800/60 backdrop-blur-sm rounded-2xl p-4 border border-white/10 space-y-3">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Thống Kê</h3>
            {[
              { label: 'Tổng ván', value: user?.games_played || 0, color: '#60a5fa' },
              { label: 'Ván thắng', value: user?.games_won || 0, color: '#10b981' },
              { label: 'Tỷ lệ thắng', value: user?.games_played > 0 ? `${Math.round((user.games_won / user.games_played) * 100)}%` : '—', color: '#f59e0b' },
              { label: 'ELO', value: user?.stats?.elo_rating || 1000, color: '#a855f7' },
            ].map(stat => (
          ...
          {/* Challenge Card */}
          <div className="bg-dark-800/60 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
            ...
            {[
              ...
              <div key={i} className="mb-3 last:mb-0">
                ...
                <div className="h-1.5 bg-dark-600 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-pink-500 to-orange-400 rounded-full transition-all" style={{ width: `${(q.progress / q.max) * 100}%` }} />
                </div>
